...
SELECT COUNT(customerID) FROM customers where last_name is not NULL;

SELECT COUNT(customerID) FROM customers where last_name is NULL;

SELECT SUM(price) FROM products;

SELECT SUM(price) FROM products where coffee_origin = "Brazil";

SELECT SUM(price)
FROM products
INNER JOIN orders ON orders.productID=products.productID where products.productID=1;

SELECT SUM(price)
FROM products
INNER JOIN orders ON orders.productID=products.productID where products.name="Latte";
...
Select sum(price) from products join 
(Select productID,first_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID where first_name="Chris";

Select max(price) from products join 
(Select productID,first_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID where first_name="Chris";

Select min(price) from products join 
(Select productID,first_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID where first_name="Chris";

Select avg(price) from products join 
(Select productID,first_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID where first_name="Chris";

SELECT count(productID),coffee_origin FROM products group by coffee_origin;

Select first_name,count(last_name) as totalorder from customers join orders on customers.customerID = orders.customerID group by first_name having totalorder>5;

Select products.productID,name,count(price) as nooforder from products join 
(Select productID,first_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID group by name;

Select customers.customerID, first_name,count(last_name) as totalorder from customers join orders on customers.customerID = orders.customerID group by first_name;

Select products.productID,name,price,coffee_origin,count(price) as nooforde from products join 
(Select productID,first_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID where price>3 group by name order by count(price) desc ;

Select customerID,first_name,max(price) from products join 
(Select customers.customerID,productID,first_name,last_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID group by customerID order by customerID;

Select customerID,first_name,avg(price) from products join 
(Select customers.customerID,productID,first_name,last_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID group by customerID order by customerID,avg(price);

Select customerID,first_name,max(price)-min(price) as pricespread from products join 
(Select customers.customerID,productID,first_name,last_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID group by customerID order by pricespread;

Select customerID,first_name from products  join 
(Select customers.customerID,productID,first_name,last_name from customers join orders on customers.customerID = orders.customerID ) as custOrder
on products.productID=custOrder.productID group by customerID having avg(price) > 3 order by customerID;
